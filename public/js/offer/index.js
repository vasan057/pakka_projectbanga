...
$(function(){
    ...
    $('#myModal').on('show.bs.modal',function(e){
        ...
        if(type == 'edit'){
            ...
            $('#myModal .modal-title').html('Update Counterprice Details');
            $('#myModal #updlocation').val('Update Details');
            $('#counter-price').show();
            
            getOrderDetails(url,location_url,is_ubp);
            
        }else if(type == 'insert'){
            $('#myModal form').attr('action',url);
            $("#myModal form input[name='_method']").remove();
            $('#myModal .modal-title').html('Create Offer Details');
...
function getOrderDetails(url,location_url,is_ubp){
    $.ajax({
        type : 'get',
        url : url,
        success : function(res){
            if(res.error){
                ...
                $('#price').val(list.order_for_price);
                $('#baseprice').val(list.order_price);
                // set arthiya default select
                setLocation(list.mandi_id,location_url,list.destination_id,function(){ $('#delivery_location').val(list.destination_id);  });
                var arthiya_url =  $('#mandi_id').attr('data-arthiya');
                setArthiya(list.mandi_id,arthiya_url,list.user_id,function(){
                    $('#pakka_arthiya_id').val(list.user_id);
                });
                if(is_ubp){
                    $('#myModal form').append("<input type='hidden' name='ubp' value='1'");
                    $('#mandi_id,#reference,#price,#pakka_arthiya_id,#delivery_location').prop('disabled',true);
                }else{
                    $('#mandi_id,#reference,#price,#pakka_arthiya_id,#delivery_location').prop('disabled',false);
                    $('#myModal form input[name=ubp]').remove();
                }
            }
        }
});
