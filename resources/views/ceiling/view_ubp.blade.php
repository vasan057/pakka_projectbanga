...
<div id="page-wrapper">
            <div class="row">
                ...
                <div class="alert alert-warning">
                   <string> Dear User, UBP Team has frozen/ Notified ceiling price.</string>
                </div>
                @endif
                @if( session('error')) 
                <div class="alert alert-danger">
                   <string> {{session('error')}}</string>
                </div>
                @endif
                @if( session('success')) 
                <div class="alert alert-success">
                   <string> {{session('success')}}</string>
                </div>
                @endif
                <div class="panel panel-default">
                    <div class="panel-body" style="color:#264595"><Strong>Ceilings Details</Strong></div>
                </div>
                    <div class='col-md-12' @if(!count($ceilings)) style="pointer-events: none;opacity: 0.6;" @endif>
                    ...
                    <a style="margin-left:  10px;margin-right: 10px;float: right;" href="{{url('ceiling/un-freeze')}}" id='unfreeze_all' class="btn btn-danger @if((count($ceilings) >0 && !$ceilings->sum('isFrozen')) || $notified == 'yes') disabled @endif">UnFreeze All</a>
                    <a style="margin-left:  10px;margin-right: 10px;float: right;" href="{{url('ceiling/freeze')}}" id='freeze_all' class="btn btn-success  @if($ceilings->sum('isFrozen') || $notified == 'yes') disabled @endif">Freeze All</a>
                    ...
                    <button class="btn btn-norm" type="button">
                                ...
                                @foreach ($ceilings as $ceiling)
                                    ...
                                    <td>{{ $ceiling->mandi->short_name or '' }}</td>
                                        ...
                                        <td>
                                                ...
                                                <button class="btn btn-norm btn-sm set-price" data-url="{{url('ceiling/set-price')}}" id="{{$ceiling->id}}" @if(!$ceilings->sum('isFrozen')) disabled @endif>Set</button>
                                        </td> 
                                        <td>
                                        <button class="btn btn-norm btn-sm set-notify" data-url="{{url('ceiling/set-notify')}}" data-id="{{$ceiling->id}}" @if(!$ceilings->sum('isFrozen')) disabled @endif>
                                            Notify
                                        </button>
                                        </td>
                                        </tr>
